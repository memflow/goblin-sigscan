...
use goblin_sigscan::pattern::Atom;
use goblin_sigscan::{MappedAddressView, elf::ElfFile, mach::MachFile, pe64::PeFile};
...
fn find_match_at<const N: usize, B: goblin_sigscan::BinaryView>(
    matches: &mut goblin_sigscan::Matches<'_, '_, B>,
    expected_start: u64,
) -> [u64; N] {
    let mut save = [0u64; N];
    loop {
...
fn pe32_fixture_is_rejected_by_pe64_parser() {
    ...
    assert!(matches!(err, goblin_sigscan::pe64::PeError::NotPe64));
}
...
fn elf_scanner_byte_and_save_match_known_prologue() {
    use goblin_sigscan::pattern as pat;
    let bytes = fixture_bytes(ELF64_FIXTURE);
