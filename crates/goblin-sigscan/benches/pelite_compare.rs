...
impl PatternCase {
    fn parse(label: &'static str, source: &'static str) -> Self {
        let goblin_atoms =
            gl_pattern::parse(source).expect("goblin-sigscan benchmark pattern should parse");
        let pelite_atoms =
...
fn bench_pe64_goblin_vs_pelite(c: &mut Criterion) {
    ...
    let goblin_file =
        GoblinPeFile::from_bytes(&bytes).expect("PE fixture should parse with goblin-sigscan");
    ...
    for case in &cases {
        first_group.bench_with_input(
            BenchmarkId::new("goblin-sigscan", case.label),
            case,
            |b, case| {
                b.iter_batched_ref(
                    || [0u64; SAVE_SLOTS],
