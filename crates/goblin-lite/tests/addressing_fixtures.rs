...
// ── ELF scanner: Byte / Save (baseline) ──────────────────────────────────────
...
fn elf_scanner_byte_and_save_match_known_prologue() {
    ...
    let mut matches = file.scanner().matches_code(pat!(
        "55 41 57 41 56 41 55 41 54 53 48 83"
    ));
    ...
    assert_eq!(mid[1], 0x1f951, "Save after first byte should capture cursor at byte 1");
}

// ── ELF scanner: Skip / SkipRange ────────────────────────────────────────────
...
fn elf_scanner_skip_bridges_fixed_gap() {
    ...
    let mut matches = file.scanner().matches_code(
        &[Atom::Save(0), Atom::Byte(0x48), Atom::Byte(0x8b), Atom::Skip(6), Atom::Byte(0x48), Atom::Byte(0x89)],
    );
...
fn elf_scanner_aligned_matches_known_aligned_prologue() {
    ...
    let mut matches = file.scanner().matches_code(pat!(
        "@4 55 41 57 41 56 41 55 41 54 53 48 83"
    ));
    let found = find_match_at::<1, _>(&mut matches, 0x1f950);
    assert_eq!(found[0], 0x1f950);
...
fn elf_scanner_back_rewinds_cursor() {
    ...
    let pat = [
        Atom::Save(0),
        Atom::Byte(0x55), Atom::Byte(0x41), Atom::Byte(0x57), Atom::Byte(0x41),
        Atom::Byte(0x56), Atom::Byte(0x41), Atom::Byte(0x55), Atom::Byte(0x41),
        Atom::Back(6),
        Atom::Byte(0x57), Atom::Byte(0x41), Atom::Byte(0x56),
    ];
    let mut save = [0u64; 1];
    let mut matches = file.scanner().matches_code(&pat);
...
// ── ELF scanner: Case / Break (alternatives) ─────────────────────────────────
...
fn elf_scanner_case_break_matches_both_alternatives() {
    ...
    while matches.next(&mut save) {
        if save[0] == 0x21b15 { found_32bit = true; }
        if save[0] == 0x1f631 { found_64bit = true; }
        if found_32bit && found_64bit { break; }
    }

    assert!(found_32bit, "32-bit `test eax,eax` (85 c0) at VA 0x21b15 should match");
    assert!(found_64bit, "64-bit `test rax,rax` (48 85 c0) at VA 0x1f631 should match");
}

// ── ELF scanner: Push + Pop (${...} sub-pattern) ─────────────────────────────
...
// ── PE64 scanner: Byte / Save (baseline) ────────────────────────────────────
...
fn pe64_scanner_aligned_only_matches_at_aligned_rvas() {
    ...
    while matches.next(&mut save) {
        assert_eq!(save[0] % 16, 0, "RVA 0x{:x} is not 16-byte aligned", save[0]);
    }
}
...
fn pe64_scanner_case_break_matches_both_alternatives() {
    ...
    while matches.next(&mut save) {
        if save[0] == 0x106c { found_32bit = true; }
        if save[0] == 0x10a3 { found_64bit = true; }
        if found_32bit && found_64bit { break; }
    }

    assert!(found_32bit, "32-bit `test eax,eax` (85 c0) at RVA 0x106c should match");
    assert!(found_64bit, "64-bit `test rax,rax` (48 85 c0) at RVA 0x10a3 should match");
}
